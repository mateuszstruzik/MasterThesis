...
{
    public class PostgreSQLServerOuterJoin40TableDbENTITY
    {
        public IEnumerable<ModelFor40TableOuterJoin> GetOuterJoin()
        {
            IEnumerable<ModelFor40TableOuterJoin> result;
            using (var context = new PostgreSQLContextFor40TableDB())
            {
                result = context.Database.SqlQuery<ModelFor40TableOuterJoin>(SqQueries.OuterJoin40Table).ToList();
            }
